...
ncclResult_t ncclTopoGetAlgoTime(struct ncclInfo* info, int algorithm, int protocol, float* time) {
  ...
  *time = lat + (info->nBytes) / (1000 * bw);
  if ((info->coll == ncclFuncAllReduce) && ((info->count % info->comm->nRanks) == 0) && (info->sendbuff == info->recvbuff) && (info->nBytes > 0) && (info->nBytes < 100*1024) && (info->comm->scclAlgo.ngpus == info->comm->nRanks)){
