...
ncclResult_t ncclTopoTuneModel(struct ncclComm* comm, int minCompCap, int maxCompCap, struct ncclTopoGraph* treeGraph, struct ncclTopoGraph* ringGraph, struct ncclTopoGraph* collNetGraph) {
  ...
  for (int coll=0; coll<NCCL_NUM_FUNCTIONS; coll++) {
    ...
    for (int a=0; a<NCCL_NUM_ALGORITHMS; a++) {
      if (coll == ncclFuncAllToAll || coll == ncclFuncCustomCollective || coll == ncclFuncCustomCollective2D || a == NCCL_ALGO_SCCL) {
        ...
        for (int p=0; p<NCCL_NUM_PROTOCOLS; p++) {
          if ((coll == ncclFuncAllToAll || coll == ncclFuncAllGather || coll == ncclFuncReduceScatter || coll == ncclFuncAllReduce) && a == NCCL_ALGO_SCCL && p == comm->scclAlgo.protocol){
            ...
            //Set all protocols for NCCL_ALGO_SCCL for ncclFuncCustomCollective and ncclFuncCustomCollectiveD to 0 because XML is loaded later.
