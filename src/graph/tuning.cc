...
ncclResult_t ncclTopoGetAlgoTime(struct ncclInfo* info, int algorithm, int protocol, float* time) {
  ...
  *time = lat + (info->nBytes) / (1000 * bw);
  if ((info->coll == ncclFuncAllReduce) && ((info->count % info->comm->nRanks) == 0) && (info->sendbuff == info->recvbuff) && (info->nBytes > 0) && (info->nBytes < 220*1024) && (info->comm->scclAlgo.ngpus == info->comm->nRanks)){
    if (algorithm != NCCL_ALGO_SCCL && info->comm->bandwidths[ncclFuncAllReduce][NCCL_ALGO_SCCL][NCCL_PROTO_LL] > 0){
	*time = -1.0; return ncclSuccess;
    }
  } else if (algorithm == NCCL_ALGO_SCCL){
	*time = -1.0; return ncclSuccess;
  }
  return ncclSuccess;
}
