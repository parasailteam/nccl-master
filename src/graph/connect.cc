...
ncclResult_t ncclTopoPostset(struct ncclComm* comm, int* firstRanks, int* treePatterns, struct ncclTopoRanks** allTopoRanks, int* rings, int maxSCCLNChannels) {
  ...
  int c;
  for (c=nChannels; c<std::max(ncclMinNchannels(),maxSCCLNChannels); c++) {
