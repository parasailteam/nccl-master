...
for node in range(nnodes):
    for g in range(ngpuspernode):
        ...
        for ch in range(instances):
            ...
            print('      <step s="0" type="s" srcbuf="s" srcoff="{}" dstbuf="s" dstoff="{}" cnt="{}" depid="{}" deps="{}" hasdep="0"/>'.format(0, ngpuspernode**2, ngpuspernode**2, 2+2*g, ngpuspernode-1))
            print('    </tb>')
            tbindex+=1
        for ch in range(instances):
            ...
            if withinnodenghr == g:
                for ch in range(instances):
                    ...
                    for j in range(ngpuspernode):
                        if j != g:
                            print('      <step s="{}" type="nop" srcbuf="i" srcoff="0" dstbuf="o" dstoff="0" cnt="0" depid="{}" deps="{}" hasdep="{}"/>'.format(step, (2*j+2+1) if j < g else (2*j+2), 0, 1 if step == 1+ngpuspernode-2 else 0))
                            step += 1
                ...
                for ch in range(instances):
                    ...
                    print('      <step s="1" type="s" srcbuf="i" srcoff="{}" dstbuf="o" dstoff="{}" cnt="{}" depid="-1" deps="-1" hasdep="0"/>'.format(node*ngpuspernode+withinnodenghr, node*ngpuspernode+g, 1))
                    step = 2
                    for j in range(ngpuspernode):
                ...
                for ch in range(instances):
                    ...
                    print('      <step s="0" type="r" srcbuf="i" srcoff="{}" dstbuf="s" dstoff="{}" cnt="{}" depid="-1" deps="-1" hasdep="1"/>'.format(nghrNode*ngpuspernode, withinnodenghr*ngpuspernode, ngpuspernode))
                    print('      <step s="1" type="r" srcbuf="i" srcoff="{}" dstbuf="o" dstoff="{}" cnt="{}" depid="-1" deps="-1" hasdep="0"/>'.format(node*ngpuspernode+g, node*ngpuspernode+withinnodenghr, 1))
                    step = 2
                    for j in range(ngpuspernode):
                        print('      <step s="{}" type="r" srcbuf="s" srcoff="{}" dstbuf="o" dstoff="{}" cnt="{}" depid="-1" deps="-1" hasdep="0"/>'.format(step, ngpuspernode**2+j*ngpuspernode+g, withinNghrNode*ngpuspernode+j, 1))
                        step += 1
                    print('    </tb>')
                    tbindex+=1
        print('  </gpu>')
