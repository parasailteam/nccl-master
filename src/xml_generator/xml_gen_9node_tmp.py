...
print('<algo name="test" nchunksperloop="{}" nchannels="{}" proto="Simple">'.format(nchunksperloop, 2*instances))
...
# --------------------------------
        for withinnodenghr  in range(ngpuspernode):
            ...
            if withinnodenghr == g:
                ...
                for ch in range(instances):
                    print('    <tb id="{}" send="{}" recv="-1" chan="{}">'.format(tbindex, node*ngpuspernode+withinnodenghr, instances+ch))
                    step = 0
                    print('      <step s="{}" type="s" srcbuf="i" srcoff="{}" dstbuf="o" dstoff="{}" cnt="{}" depid="-1" deps="-1" hasdep="0"/>'.format(step, instances*(node*ngpuspernode+withinnodenghr)+ch, instances*(node*ngpuspernode+g)+ch, 1))
                    step += 1
                    for j in range(ngpuspernode):
                    ...
                    tbindex+=1
                for ch in range(instances):
                    print('    <tb id="{}" send="-1" recv="{}" chan="{}">'.format(tbindex, node*ngpuspernode+withinnodenghr, instances+ch))
                    step = 0
                    print('      <step s="{}" type="r" srcbuf="i" srcoff="{}" dstbuf="o" dstoff="{}" cnt="{}" depid="-1" deps="-1" hasdep="0"/>'.format(step, instances*(node*ngpuspernode+g)+ch, instances*(node*ngpuspernode+withinnodenghr)+ch, 1))
                    step += 1
                    for j in range(ngpuspernode):
