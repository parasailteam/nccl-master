...
for node in range(nnodes):
    for g in range(ngpuspernode):
        ...
        for ch in range(instances):
            ...
            print('      <step s="0" type="s" srcbuf="s" srcoff="{}" dstbuf="s" dstoff="{}" cnt="{}" depid="-1" deps="-1" hasdep="0"/>'.format(ch*ngpuspernode**2, instances*ngpuspernode**2+ch*ngpuspernode**2, ngpuspernode**2, instances*(2+2*g)+ch, ngpuspernode))
            print('    </tb>')
            tbindex+=1
        for ch in range(instances):
