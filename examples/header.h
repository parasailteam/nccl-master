...
static float absRelDiff(float u, float v) {
  return abs((u-v)/u);
}
static bool eqFloat(float u, float v) {
  if (u == 0.0f || v == 0.0f)
    return u == v;
...
template<class T>
static void cudaMemRandInt(T* dst, size_t nelems)
{
...
__global__ static void floatToHalfArrayKernel(half* h, float* f, size_t num)
{
...
template<class T>
static void memset_value(T*f, T v, size_t nelems)
{
...
template<class T>
static void memset_identity_values(T*f, size_t nelems)
{
  T* h_buff = (T*)malloc(sizeof(T)*nelems);
...
static void halfToFloatArray(float* f, half* h, size_t num)
{
...
static void floatToHalfArray(half* h, float* f, size_t num)
{
  for (size_t i = 0; i < num; i++) {
...
static void cudaMemcpyHalfDevice2FloatHost(float* hostFloatArray, half* deviceHalfArray, size_t nelems)
{
