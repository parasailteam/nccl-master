...
def ring_16():
    with open("allreduce_ring_%d_ranks_%d_channel_2D_chunks.xml"%(num_gpus, nchannels), "w") as f:
        ...
        for g in range(num_gpus):
            ...
            for c in range(nchannels):
                ...
                for s in range(num_gpus):
                    ...
                    if s == 0:
                        f.write('      <step s="{}" type="s" srcbuf="i" srcoff="{}" dstbuf="o" dstoff="0" cnt="1" depid="-1" deps="-1" hasdep="0"/>\n'.format(s, src_off)) # TODO check
                    elif s == num_gpus-1:
                        f.write('      <step s="{}" type="rrcs" srcbuf="i" srcoff="{}" dstbuf="o" dstoff="{}" cnt="1" depid="-1" deps="-1" hasdep="0"/>\n'.format(s, src_off, src_off))
                    else:
                        f.write('      <step s="{}" type="rrs" srcbuf="i" srcoff="{}" dstbuf="o" dstoff="0" cnt="1" depid="-1" deps="-1" hasdep="0"/>\n'.format(s, src_off))
