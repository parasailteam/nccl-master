...
#include "cutlass/gemm/device/scclgemm.h"
...
using SCCLGemm = cutlass::gemm::device::SCCLGemm<ElementInputA,
                                         LayoutInputA,
                                         ElementInputB,
                                         LayoutInputB,
                                         ElementOutput,
...
float cutlassGeMM(const int length_m, const int length_n, const int length_k, int rank,
  ...
  auto vvv = std::vector<std::vector<NCCLChunk>>();
  int chunkSize;
  ...
  status = gemm_op.initialize(arguments, 0, workspace.get());
  CUTLASS_CHECK(status);
